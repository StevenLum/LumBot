...
class Music2(commands.Cog, name='Musique'):
    ...
    def search(author, arg):
        with YoutubeDL(Music2.YDL_OPTIONS) as ydl:
            try: requests.get(arg)
            except: info = ydl.extract_info(f"ytsearch:{arg}", download=False)['entries'][0]
            else: info = ydl.extract_info(arg, download=False)

        embed = (Embed(title='🎵 Musique en cours :', description=f"[{info['title']}]({info['webpage_url']})", color=0x3498db)
                .add_field(name='Duration', value=Music2.parse_duration(info['duration']))
    ...
    async def play(self, ctx, *, video: str):
        ...
        voice = get(self.bot.voice_clients, guild=ctx.guild)
        song = Music2.search(ctx.author.mention, video)
        ...
        if not voice.is_playing():
            ...
            voice.play(FFmpegPCMAudio(song['source'], **Music2.FFMPEG_OPTIONS), after=lambda e: self.play_next(ctx))
            voice.is_playing()
        else:
